...
class OverlayPermissionGuideActivity : Activity() {
        ...
        try {
            ...
            findViewById<TextView>(R.id.guide_title_text)?.text =
                intent.getStringExtra(
                    FlutterBoomNotificationPluginsPlugin.EXTRA_OVERLAY_PERMISSION_GUIDE_TITLE,
            ...
            findViewById<TextView>(R.id.guide_description_text)?.text =
                intent.getStringExtra(
                    FlutterBoomNotificationPluginsPlugin.EXTRA_OVERLAY_PERMISSION_GUIDE_DESC,
                )?.takeUnless { it.isBlank() }
                    ?.replaceAppNamePlaceholder(appName)
    ...
    private fun resolveLayoutId(): Int {
        val layoutName =
            intent.getStringExtra(
                FlutterBoomNotificationPluginsPlugin.EXTRA_OVERLAY_PERMISSION_GUIDE_LAYOUT,
            )?.trim()
                ?.takeUnless { it.isBlank() }
