...
internal class NotificationRemoteConfigManager(
    ...
    fun activate(config: JSONObject) {
        activeConfig = config
        preferences.edit()
            .putBoolean(KEY_NOTIFICATIONS_ENABLED, config.optBoolean("enabled", false))
            .putLong(
                KEY_REFRESH_INTERVAL_SECONDS,
                config.optLong("refresh_interval_seconds", 0L).coerceAtLeast(0L),
            )
            .putLong(
            ...
            .putString(
                KEY_MEDIA_NOTIFICATION_ARRAY_JSON,
    ...
    companion object {
        ...
        fun getRefreshIntervalSeconds(context: Context): Long =
            context.applicationContext
                .getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                .getLong(KEY_REFRESH_INTERVAL_SECONDS, 0L)

        fun getRefreshDurationSeconds(context: Context): Long =
            context.applicationContext
                .getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        ...
        fun getMediaNotificationArray(context: Context): JSONArray? =
            context.applicationContext
                .getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                .getString(KEY_MEDIA_NOTIFICATION_ARRAY_JSON, null)
                ?.let { runCatching { JSONArray(it) }.getOrNull() }
        ...
        const val KEY_MEDIA_NOTIFICATION_ARRAY_JSON = "media_notification_array_json"
